...
inline void CalcConvertedRef(const DiffractionExperiment &experiment, const JungfrauCalibration &calib,
    ...
    for (size_t i = 0; i < pixels; i++) {
        ...
        switch (gainbits) {
            case 0:
                ...
                expected = (adcfloat - calib.PedestalFP(0)[i]) * one_over_gain;
                gain_mask[i] = 0;
                break;
            case 0x4000:
                gain = gain_arr[i % RAW_MODULE_SIZE + RAW_MODULE_SIZE];
                one_over_gain = ((gain * energy != 0.0) ? 1.0 / (gain * energy) : 0.0);
                expected = (adcfloat - calib.PedestalFP(1)[i]) * one_over_gain;
                gain_mask[i] = 1;
            ...
            case 0xc000:
                ...
                one_over_gain = ((gain * energy != 0.0) ? 1.0 / (gain * energy) : 0.0);
                expected = (adcfloat - calib.PedestalFP(2)[i]) * one_over_gain;
                gain_mask[i] = 3;
...
template <class T> double CheckConversion(const DiffractionExperiment &experiment, const JungfrauCalibration &calib,
    ...
    CalcConvertedRef(experiment, calib, raw, conversion_ref.data(), gain_mask_ref.data(), gain_file, pixels);
...
template <class T> double CheckConversionWithGeomTransform(const DiffractionExperiment &experiment, const JungfrauCalibration &calib,
        const uint16_t *raw, T *converted, const std::string &gain_file = "../../tests/test_data/gainMaps_M049.bin") {
    ...
    CalcConvertedRef(experiment, calib, raw, conversion_ref.data(), gain_mask_ref.data(),
                     gain_file, experiment.GetModulesNum() * RAW_MODULE_SIZE);
